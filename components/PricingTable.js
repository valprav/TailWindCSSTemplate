        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {tiers.map((tier, index) => (
            <div key={index} className="bg-white p-8 rounded-lg shadow flex flex-col">
              ...
              <p className="mt-4 text-3xl font-extrabold text-indigo-600">{tier.price}</p>
              <ul className="mt-6 space-y-2 flex-1">
                {tier.features.map((feature, i) => (
                  <li key={i} className="text-gray-600">{feature}</li>
                ))}
              </ul>
              <a href="#" className="mt-8 inline-block px-6 py-3 bg-indigo-600 text-white font-semibold rounded-md hover:bg-indigo-700">{tier.cta}</a>
            </div>
          ))}
        </div>
      </div>
